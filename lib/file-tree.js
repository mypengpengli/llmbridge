class FileTree {
    ...
    this.shadowRoot.innerHTML = `
      ...
      <div class="aifr-sidebar" id="sidebar">
        <div class="aifr-header">
          <span class="aifr-title">${I18n.t('appName')}</span>
          <div class="aifr-header-actions">
            <button class="aifr-btn" id="refresh-btn" title="${I18n.t('refresh')}">↻</button>
            <button class="aifr-btn" id="close-btn" title="${I18n.t('close')}">✕</button>
          </div>
        </div>
        <div class="aifr-inserted" id="inserted-panel" style="display:none;">
          <div class="aifr-inserted-header" id="inserted-toggle">
            <span>${I18n.t('added')} (<span id="inserted-count">0</span>)</span>
            <button class="aifr-btn" id="inserted-clear" title="${I18n.t('clear')}">✕</button>
          </div>
          <div class="aifr-inserted-list" id="inserted-list"></div>
        </div>
        <div class="aifr-search">
          <input type="text" id="search-input" placeholder="${I18n.t('searchFiles')}" />
        </div>
        <div class="aifr-status-bar">
          <span class="aifr-status" id="status"></span>
          <button class="aifr-btn" id="switch-root-btn" title="${I18n.t('switchDirectory')}">📂</button>
        </div>
        <div class="aifr-tree" id="tree"></div>
        <div class="aifr-templates" id="templates-panel">
          <div class="aifr-templates-header" id="templates-toggle">${I18n.t('quickPrompts')}</div>
          <div class="aifr-builtins">
            <button class="aifr-action-btn" id="insert-structure-btn">${I18n.t('currentDirectoryStructure')}</button>
          </div>
          <div class="aifr-templates-list" id="templates-list"></div>
          <div class="aifr-templates-add">
            <input type="text" id="template-input" placeholder="${I18n.t('addQuickPrompt')}" />
  ...
  _renderInserted() {
    ...
    list.innerHTML = this.insertedFiles.map((f, i) => {
      ...
      return `
        <div class="aifr-inserted-item">
          ...
          <span class="undo" data-index="${i}">${I18n.t('undo')}</span>
        </div>
      `;
    }).join('');
    ...
    list.querySelectorAll('.undo').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        this.showToast(I18n.t('undoneFromChat'));
      });
    });
  }
  ...
  async refresh() {
    ...
    if (mode === 'disconnected') {
      status.textContent = I18n.t('notConnected');
      ...
      const btnText = hasSavedHandle
        ? I18n.t('reauthorizeFolderValue', this.fileAccess.dirHandle.name)
        : I18n.t('selectFolder');

      tree.innerHTML = `
        <div class="aifr-empty">
          <p>${hasSavedHandle ? I18n.t('reauthorizationRequired') : I18n.t('nativeDisconnected')}</p>
          ...
          <p style="margin-top: 8px; font-size: 11px; color: #6c7086;">
            ${hasSavedHandle ? I18n.t('clickToRestoreAccess') : I18n.t('installNativeHint')}
          </p>
        </div>
      `;
    ...
    if (result.success) {
      ...
      status.textContent = `${mode === 'native' ? I18n.t('nativeService') : I18n.t('browserApi')} | ${this.fileAccess.rootDir || I18n.t('rootDirectory')}`;
      this._renderTree(tree, this.treeData);
    } else if (result.error && result.error.includes('not configured')) {
      this._renderSetRootPanel(I18n.t('nativeConnected'));
    } else {
      status.textContent = I18n.t('errorValue', result.error);
      status.className = 'aifr-status error';
      tree.innerHTML = `<div class="aifr-empty">${I18n.t('loadFailed')}</div>`;
    }
  }
  ...
  _renderTree(container, entries) {
    ...
    if (!entries || entries.length === 0) {
      container.innerHTML = `<div class="aifr-empty">${I18n.t('emptyDirectory')}</div>`;
      return;
    }
  ...
  _createNode(entry) {
    ...
    if (entry.type === 'directory') {
      ...
      node.addEventListener('click', async () => {
        ...
        if (!loaded && expanded) {
          loaded = true;
          children.innerHTML = `<div class="aifr-loading">${I18n.t('loading')}</div>`;
          const result = await this.fileAccess.listDirectory(entry.path);
          if (result.success) {
            this._renderTree(children, result.entries);
          } else {
            children.innerHTML = `<div class="aifr-loading">${I18n.t('loadFailed')}</div>`;
          }
        }
      });
    } else {
      ...
      node.addEventListener('click', async () => {
        ...
        if (isBinary) {
          ...
          if (result.success) {
            ...
            this.showToast(I18n.t('uploadedFile', [entry.name, sizeStr]));
          } else {
            this.showToast(I18n.t('uploadFailed', result.error), 'error');
          }
        } else {
          ...
          if (result.success) {
            ...
            const sizeStr = result.size > 1024 ? `${(result.size / 1024).toFixed(1)}KB` : `${result.size}B`;
            this.showToast(I18n.t('insertedFile', [entry.name, sizeStr]));
          } else {
            this.showToast(I18n.t('readFailed', result.error), 'error');
          }
        }
      });
    }
  ...
  async _showSwitchRoot() {
    ...
    try {
      ...
      if (resp.success && resp.roots && resp.roots.length > 0) {
        recentHtml = `<div style="margin-top:10px;font-size:11px;color:#6c7086;">${I18n.t('recentlyUsed')}</div>` +
    ...
    tree.innerHTML = `
      <div class="aifr-empty">
        <p>${I18n.t('switchWorkingDirectory')}</p>
    ...
    const doSwitch = async (dir) => {
      ...
      if (resp.success) {
        this.showToast(I18n.t('switchedTo', resp.rootDir));
        this.fileAccess.fileIndex = [];
        await this.refresh();
        this.fileAccess.listAllFiles(8);
      } else {
        this.showToast(resp.error || I18n.t('switchFailed'), 'error');
      }
    };
  ...
  _onSearch(query) {
    ...
    if (allFiles.length === 0) {
      tree.innerHTML = `<div class="aifr-loading">${I18n.t('indexing')}</div>`;
    ...
    if (matches.length === 0) {
      tree.innerHTML = `<div class="aifr-empty">${I18n.t('noMatches')}</div>`;
      return;
    }
  ...
  async _showSwitchRoot() {
    ...
    if (mode === 'filesystem-api' || mode === 'disconnected') {
      ...
      if (result.success) {
        ...
        this.showToast(I18n.t('switchFailedValue', result.error), 'error');
      }
      return;
    }

    this._renderSetRootPanel(I18n.t('switchWorkingDirectory'));
  }
  ...
  _renderSetRootPanel(statusText) {
    ...
    tree.innerHTML = `
      <div class="aifr-empty">
        <p>${I18n.t('setWorkingDirectory')}</p>
        <input type="text" id="aifr-root-input" placeholder="${I18n.t('pathExample')}" style="
          width: 100%;
          margin-top: 12px;
          padding: 8px 10px;
          border: 1px solid #45475a;
    ...
    setRootBtn.addEventListener('click', async () => {
      ...
      setRootBtn.textContent = I18n.t('setting');
      const resp = await this.fileAccess.setRoot(dir);
      if (resp.success) {
        this.treeData = [];
        this.fileAccess.fileIndex = [];
        ...
        setRootBtn.textContent = I18n.t('confirm');
        this.showToast(resp.error || I18n.t('setFailed'), 'error');
      }
    });
  ...
  async _insertDirectoryStructure(button) {
    if (this.fileAccess.getMode() === 'native' && !this.fileAccess.rootDir) {
      this.showToast(I18n.t('setWorkingDirectoryFirst'), 'error');
      await this.refresh();
      return;
    }
    ...
    button.textContent = I18n.t('generating');
    ...
    try {
      ...
      if (!result.success) {
        this.showToast(I18n.t('generateFailedValue', result.error), 'error', 5000);
        return;
      }
      files = result.files || [];
      ...
      if (files.length === 0) {
        this.showToast(I18n.t('noInsertableFiles'), 'error');
        return;
      }

      const content = this._formatDirectoryStructure(files, truncated);
      this.onFileSelect(content);
      this.recordInsert(I18n.t('currentDirectoryStructure'), content);
      this.showToast(I18n.t('insertedDirectoryStructure', String(files.length)));
    } catch (err) {
      this.showToast(I18n.t('generateFailedValue', err.message), 'error', 5000);
    } finally {
  ...
  _formatDirectoryStructure(files, truncated) {
    ...
    let content = `${I18n.t('directoryStructureIntro')}\n\n\`\`\`text\n${lines.join('\n')}\n\`\`\``;
    if (truncated) {
      content += `\n\n${I18n.t('directoryStructureTruncated')}`;
    }
    return content;
  }
  ...
  _getRootDisplayName() {
    const rootDir = this.fileAccess.rootDir || I18n.t('currentDirectory');
    const normalized = rootDir.replace(/[\\/]+$/, '');
    return normalized.split(/[\\/]/).pop() || normalized || I18n.t('currentDirectory');
  }
  ...
  async _loadTemplates() {
    try {
      ...
      this._templates = data.templates || [
        I18n.t('defaultPromptAnalyze'),
        I18n.t('defaultPromptExplain'),
        I18n.t('defaultPromptRefactor')
      ];
  ...
  _renderTemplates() {
    ...
    list.querySelectorAll('.aifr-template-item').forEach(item => {
      item.addEventListener('click', (e) => {
        ...
        this.showToast(I18n.t('insertedPrompt'));
      });
    });
  }
