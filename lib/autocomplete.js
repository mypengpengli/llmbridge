class Autocomplete {
  ...
  _renderResults() {
    if (this.results.length === 0) {
      this.popup.innerHTML = `
        <div style="padding: 12px; color: #6c7086; text-align: center;">
          ${this.query ? I18n.t('noMatchingFiles') : I18n.t('searchByFilename')}
        </div>
      `;
      this.popup.style.display = 'block';
      return;
    ...
    this.popup.innerHTML = this.results.map((r, i) => {
      ...
      const icon = isDir ? '📁' : '📄';
      const label = isDir ? `${name}/ (${I18n.t('entireFolder')})` : name;
      return `
  ...
  async _insertDirectory(dirPath) {
    ...
    const summary = result.loadedFiles < result.totalFiles
      ? `\n\n> ${I18n.t('loadedFilesSummary', [String(result.loadedFiles), String(result.totalFiles), (result.totalSize / 1024).toFixed(1)])}\n`
      : '';
